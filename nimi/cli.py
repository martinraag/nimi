...
from nimi.route53 import create_hosted_zone, delete_hosted_zone, find_hosted_zone_id,\
    get_alias_record, get_ns_record, remove_alias_record
...
def eject(ctx, domain):
    ...
    if hosts:
        click.confirm(f'😟  Remove dyanmic DNS hosts and records: { "".join(hosts) }?', abort=True)
